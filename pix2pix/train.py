...
input_data = Dataset(data_start = 1, data_end = 299, train=True)
train_len = input_data.len()

print(train_len,"---")

generator_G = Generator(input_channel, output_channel).cuda()
discriminator_D = Discriminator(input_channel, output_channel).cuda()
...
for epoch in range(args.epoch):
    for iterate in range(train_len):
        ...
        if iterate % args.iterate == 0:
            print (iterate, epoch+1, args.epoch, loss_gen.data, loss_dis.data)
    ...
    input_data.shuffle()
print ("last", args.epoch, args.epoch, loss_gen.data, loss_dis.data)
